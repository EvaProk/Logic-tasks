...
// of HTML DOM elements and plain text. The elements that will be used are: b, i, em, div, p. 
...
// return the first element encountered where, if changed into a different element, 
...
let arrOpen = str.match(/<\w+>/g )
let arrClose = str.match(/(<\/\w+>)/g).reverse()

for(let i = arrClose.length-1; i >=0 ; i-- ){
  if(htmlObjClose[arrClose[i]] !== htmlObj[arrOpen[i]]){
   
    return htmlObj[arrOpen[i]] 
  } 
  }
  return true; 
